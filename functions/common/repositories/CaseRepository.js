...
class CaseRepository {
    ...
    async findById(caseId, scope) {
        const query = `SELECT * FROM CaseMaster WHERE CaseMasterID = ${caseId} AND ${(0, datastore_1.scopeWhereClause)(scope)}`;
    ...
    async list(scope, filters) {
        ...
        if (filters.fromDate)
            whereClause += ` AND CrimeRegisteredDate >= '${filters.fromDate}'`;
        if (filters.toDate)
            whereClause += ` AND CrimeRegisteredDate <= '${filters.toDate}'`;
        if (filters.category)
            whereClause += ` AND CrimeMajorHeadID = ${filters.category}`;
